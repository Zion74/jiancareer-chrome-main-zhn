...
class PopupManager {
  ...
  async getCurrentTab() {
    try {
      if (typeof chrome !== 'undefined' && chrome.tabs) {
        ...
        console.log('Chrome tabs API not available - using mock tab');
        return { url: 'http://localhost:8000', title: 'Test Page' };
      }
    } catch (error) {
    ...
    try {
      if (typeof chrome !== 'undefined' && chrome.storage) {
        ...
        if (result.isAuthenticated) {
          ...
          this.userInfo = result.userInfo || { username: 'user' };
        } else {
          this.currentState = 'login-initial';
        }
      } else {
  ...
  checkAuthStatusFallback() {
    try {
      ...
      if (this.userAuth) {
        this.userInfo = { username: this.userAuth.username };
  ...
  setupEventListeners() {
    ...
    if (websiteBtn) {
      websiteBtn.addEventListener('click', () => {
        window.open('https://jiancareer.com', '_blank');
      });
    }
    ...
    if (backBtn) {
      backBtn.addEventListener('click', () => {
        this.switchToState('login-initial');
      });
    }
  ...
  updateUI() {
    // 隐藏所有状态页面
    const states = ['login-initial', 'login-form', 'logged-in', 'filling', 'success', 'partial-missing', 'failed'];
    ...
    // 显示当前状态
    const stateNumber = this.getStateNumber(this.currentState);
    const currentStateEl = document.getElementById(`state-${stateNumber}`);
    if (currentStateEl) {
      currentStateEl.classList.remove('hidden');
    } else {
      console.log(`State element not found: state-${stateNumber} for ${this.currentState}`);
    }

    // 更新状态文本 - 根据不同状态更新对应的文本元素
    const statusMessages = document.querySelectorAll('.status-message');
    statusMessages.forEach(el => {
      if (el && el.closest(`#state-${this.getStateNumber(this.currentState)}`)) {
        const stateText = this.getStateText(this.currentState);
        if (stateText) {
    ...
    // 更新状态文本和特定状态的内容
    if (currentStateEl) {
      const userStatus = currentStateEl.querySelector('.username');
      console.log(`Updating state: ${this.currentState}, userStatus element:`, userStatus);
      ...
      switch (this.currentState) {
        ...
        case 'logged-in':
          if (userStatus) {
            userStatus.textContent = this.userInfo?.username || 'user6687331s';
          }
          // 更新当前状态下的用户名显示元素
          const usernameDisplay = currentStateEl.querySelector('.username');
          if (usernameDisplay) {
            usernameDisplay.textContent = this.userInfo?.username || 'user6687331s';
          }
          break;
          case 'filling':
            if (userStatus) userStatus.textContent = this.userInfo?.username || 'user6687331s';
          ...
          case 'success':
            ...
            if (this.autofillResult) {
              const successMessage = currentStateEl.querySelector('.success-message');
              if (successMessage) {
                successMessage.textContent = `成功填写 ${this.autofillResult.filledCount} / ${this.autofillResult.totalCount} 个字段`;
              }
            }
            
            this.scheduleReturnToLoggedIn();
            break;
          case 'partial-missing':
            if (userStatus) userStatus.textContent = this.userInfo?.username || 'user6687331s';
            this.updatePartialMissingMessage();
            break;
          case 'failed':
            if (userStatus) userStatus.textContent = this.userInfo?.username || 'user6687331s';
  ...
  async performLogin(username, password) {
    try {
      ...
      this.userInfo = { username: authData.username };
      
      // 切换到已登录状态
      this.switchToState('logged-in');
      return true;
    } catch (error) {
      console.error('Login error:', error);
      return false;
  ...
  sendAutofillMessage(resumeData) {
    if (typeof chrome !== 'undefined' && chrome.tabs) {
      ...
      console.log('Chrome tabs API not available - simulating autofill');
      // 模拟自动填写过程
      setTimeout(() => {
        this.handleAutofillResponse({
          success: true,
          filledCount: 8,
          totalCount: 10,
          message: '模拟填写完成'
  ...
  handleAutofillResponse(response) {
    ...
    if (response.ok) {
      ...
      // 根据填写结果选择状态
      if (response.filledCount === response.totalCount) {
        this.switchToState('success');
      } else {
        this.switchToState('partial-missing');
      }
    } else {
      // 失败：切换到失败状态
  ...
  scheduleReturnToLoggedIn() {
    ...
    this.returnTimeout = setTimeout(() => {
      if (this.currentState === 'success' || this.currentState === 'partial-missing') {
        this.currentState = 'logged-in';
  ...
  updatePartialMissingMessage() {
    const partialMessage = document.querySelector('.partial-message');
    if (partialMessage && this.autofillResult) {
      partialMessage.textContent = `已填写 ${this.autofillResult.filledCount} / ${this.autofillResult.totalCount} 个字段，部分信息缺失`;
    }
  ...
  setupMessageListener() {
    ...
    if (typeof chrome !== 'undefined' && chrome.runtime && chrome.runtime.onMessage) {
      ...
      console.log('Chrome extension API not available - running in web environment');
    }
  }
  ...
  openProfilePage() {
    ...
    if (typeof chrome !== 'undefined' && chrome.runtime) {
      ...
      console.log('Opening profile page - Chrome API not available');
      window.open('https://jiancareer.com/profile', '_blank');
    }
  }
  ...
  openFeedback() {
    if (typeof chrome !== 'undefined' && chrome.tabs) {
      ...
      console.log('Opening feedback page - Chrome API not available');
  ...
  getStateText(state) {
    const stateTexts = {
      'login-initial': '请登录以使用自动填写功能',
      'login-form': '请输入您的登录信息',
      'logged-in': '选择简历版本并开始填写',
      'filling': '正在自动填写表单...',
      'success': '填写完成！',
      'partial-missing': '部分填写完成',
      'failed': '填写失败，请重试'
    };
  ...
  showMessage(text, type = 'success') {
    ...
    if (messageEl) {
      ...
      console.log('Message element not found:', text);
    }
  }
}
