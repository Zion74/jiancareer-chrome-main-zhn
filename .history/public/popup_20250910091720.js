...
class PopupManager {
  ...
  // 处理自动填写进度消息
  handleAutofillProgress(data) {
    console.log("收到自动填写进度:", data);
    if (
      data &&
      typeof data.filled === "number" &&
      typeof data.total === "number"
    ) {
      this.autofillProgress = data;
      this.updateAutofillProgress();

      // 更新状态提示
      const progressText = `正在填写 ${data.filled}/${data.total} 个字段...`;
      this.updateStatusIndicator("processing", "自动填写中", progressText);
    }
  }

  // 处理自动填写完成消息
  handleAutofillComplete(data) {
    console.log("收到自动填写完成:", data);
    ...
    if (data && data.success) {
      this.autofillResult = {
        filledCount: data.filled || 0,
        totalCount: data.total || 0,
        detail: data.message || "自动填写完成",
      };
      ...
      this.showStatusIndicator(
        "success",
        "填写完成",
        `成功填写 ${data.filled}/${data.total} 个字段`
      );
    } else {
      // 处理失败情况
      this.handleAutofillError(
        "FILL_ERROR",
        data.message || "填写过程中发生错误"
      );
    }
  }

  // 处理表单检测消息
  handleFormsDetected(data) {
    console.log("收到表单检测结果:", data);

    if (data) {
      this.formsData = data;

      // 更新UI显示检测到的表单信息
      const formsInfo = document.querySelector(".forms-info");
      if (formsInfo) {
        formsInfo.textContent = `检测到 ${data.formsCount || 0} 个表单，${
          data.fieldsCount || 0
        } 个字段`;
      }
  ...
  updateUI() {
    ...
    // 显示当前状态
    const currentStateEl = document.getElementById(
    ...
    switch (this.currentState) {
      ...
      case 3:
        ...
        if (usernameDisplay) {
          usernameDisplay.textContent =
            this.userAuth?.username || "user6687331s";
        }
        break;
      case 4:
        statusText.textContent = this.userAuth?.username || "user6687331s";
      ...
      case 7:
        ...
        if (logoutMessageElement) {
          logoutMessageElement.textContent =
            "您已成功退出登录，感谢使用简职涯！";
        }
  ...
  openRegisterPage() {
    const signupUrl =
      "https://jiancareer.com/signup" ||
      chrome.runtime.getURL("../jiancareer-front-end-main/signup.html");
    chrome.tabs.create({ url: signupUrl });
  }

  openWebsite() {
    const homeUrl =
      "https://jiancareer.com" ||
      chrome.runtime.getURL("../jiancareer-front-end-main/index.html");
  ...
  async handleAutofill() {
    ...
    // 显示状态提示组件 - 自动填写中，确保持续显示
    this.showStatusIndicator("processing", "自动填写中", "正在分析表单结构...");

    // 确保状态指示器在填写过程中持续显示
    this.statusIndicatorVisible = true;
  ...
  async sendAutofillMessage() {
    try {
      console.log("开始发送自动填写消息");
      ...
      if (!tab) {
        ...
        this.handleAutofillError("NO_TAB", "无法获取当前标签页");
        return;
      }
      ...
      try {
        const pingResponse = await chrome.tabs.sendMessage(tab.id, pingMessage);
        console.log("收到ping响应:", pingResponse);
      } catch (pingError) {
        console.error("Ping失败:", pingError);
        this.handleAutofillError(
          "NO_CONTENT_SCRIPT",
          "无法与当前页面通信，请确认扩展已正确加载到此页面"
        );
        return;
      }
      ...
      console.log("发送自动填写消息:", message);

      const response = await chrome.tabs.sendMessage(tab.id, message);
      console.log("收到自动填写响应:", response);

      if (response) {
        this.handleAutofillResponse(response);
      } else {
        console.error("未收到响应");
        this.handleAutofillError("NO_RESPONSE", "未收到填写响应");
      }
    } catch (error) {
      console.error("发送自动填写消息失败:", error);
  ...
  handleAutofillResponse(response) {
    ...
    if (response.ok) {
      ...
      this.showStatusIndicator(
        "success",
        "填写完成",
        `已成功填写 ${response.filledCount}/${response.totalCount} 个字段`
      );

      // 标记状态指示器不再处于填写中状态
      this.statusIndicatorVisible = false;

      // 5秒后隐藏状态提示，给用户更多时间查看结果
      setTimeout(() => {
        this.hideStatusIndicator();
      }, 5000);
    } else {
  ...
  getErrorMessage(reason, detail, missingFields) {
    const errorConfigs = {
      PARTIAL: {
        title: "部分字段填写失败",
        detail: `部分字段未填：${
          missingFields ? missingFields.join(", ") : "未知字段"
        }`,
        showEditButton: true,
      },
      UNSUPPORTED_SITE: {
  ...
  updateAutofillProgress() {
    ...
    if (resumeVersionText) {
      ...
      resumeVersionText.textContent = `使用：${
        resumeNames[this.selectedResume] || "默认简历"
      }`;
    }
  }
  ...
  setupMessageListener() {
    ...
    if (
      ...
      console.log("设置Chrome扩展消息监听器");

      // 监听来自content script的消息
      chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
        console.log("Popup收到消息:", message, "from:", sender);

        try {
          switch (message.type) {
            case "AUTOFILL_PROGRESS":
              this.handleAutofillProgress(message.data);
              break;
            case "AUTOFILL_COMPLETE":
              this.handleAutofillComplete(message.data);
              break;
            case "AUTOFILL_ERROR":
              this.handleAutofillError(message.error, message.message);
              break;
            case "FORMS_DETECTED":
          ...
          if (sendResponse) {
            sendResponse({ error: error.message, timestamp: Date.now() });
          }
        }
  ...
  async getProfileData() {
    try {
      ...
      const defaultProfile =
        Object.values(profiles).find((p) => p.isDefault) ||
      ...
      return {
        ...
        city: "浙江杭州",
        location: "浙江杭州",
        github: "https://github.com/baokuangli",
        githubUrl: "https://github.com/baokuangli",
        university: "浙江大学",
        school: "浙江大学",
        major: "计算机科学与技术",
        specialty: "计算机科学与技术",
        ...
        resumeText:
          "我是一名热爱编程的软件工程师，具有丰富的前端和后端开发经验。",
        summary: "我是一名热爱编程的软件工程师，具有丰富的前端和后端开发经验。",
        workExperience: "3年软件开发经验",
        experience: "3年软件开发经验",
        education: "本科",
        degree: "本科",
        skills: "JavaScript, Python, React, Vue.js, Node.js, MySQL",
        specialties: "JavaScript, Python, React, Vue.js, Node.js, MySQL",
      };
    } catch (error) {
  ...
  // getResumeData方法作为getProfileData的别名
  async getResumeData() {
    ...
    if (success) {
      if (stats) {
        ...
        if (failed > 0) {
          // 部分成功
          this.showMessage(
            `填写完成：成功 ${filled}/${total} 个字段，${failed} 个失败`,
            "warning"
          );
        } else {
          // 完全成功
  ...
  async loadResumeData() {
    try {
      ...
      if (experienceSelect)
        experienceSelect.value = profileData.experience || "";
      ...
      if (currentCompanyInput)
        currentCompanyInput.value = profileData.currentCompany || "";
      if (expectedPositionInput)
        expectedPositionInput.value = profileData.expectedPosition || "";
      if (expectedCityInput)
        expectedCityInput.value = profileData.expectedCity || "";
      if (skillsInput) skillsInput.value = profileData.skills || "";
      if (wechatInput) wechatInput.value = profileData.wechat || "";
      if (githubInput) githubInput.value = profileData.github || "";
      if (projectExperienceTextarea)
        projectExperienceTextarea.value = profileData.projectExperience || "";
      if (selfIntroductionTextarea)
        selfIntroductionTextarea.value = profileData.selfIntroduction || "";
  ...
  async updateProfileDisplay() {
    try {
      ...
      if (phoneElement)
        phoneElement.textContent = profileData.phone || "未设置";
      if (emailElement)
        emailElement.textContent = profileData.email || "未设置";
      if (experienceElement)
        experienceElement.textContent = profileData.experience || "未设置";
      if (educationElement)
        educationElement.textContent = profileData.education || "未设置";
      if (salaryElement)
        salaryElement.textContent = profileData.salary || "未设置";
...
document.addEventListener("DOMContentLoaded", async () => {
  try {
    ...
    errorDiv.style.cssText =
      "position: fixed; top: 10px; left: 10px; right: 10px; background: #ff4444; color: white; padding: 10px; border-radius: 4px; z-index: 9999;";
