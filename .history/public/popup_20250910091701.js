...
class PopupManager {
  ...
  handleAutofillProgress(data) {
    ...
    if (data && typeof data.filled === 'number' && typeof data.total === 'number') {
  ...
  handleAutofillComplete(data) {
    ...
    if (data && data.success) {
      ...
      this.showStatusIndicator('success', '填写完成', `成功填写 ${data.filled}/${data.total} 个字段`);
    } else {
      // 处理失败情况
      this.handleAutofillError('FILL_ERROR', data.message || '填写过程中发生错误');
    }
  }
  ...
  handleFormsDetected(data) {
    ...
    if (data) {
      ...
      if (formsInfo) {
        formsInfo.textContent = `检测到 ${data.formsCount || 0} 个表单，${data.fieldsCount || 0} 个字段`;
      }
  ...
  async sendAutofillMessage() {
    try {
      ...
      try {
        ...
        this.handleAutofillError('NO_CONTENT_SCRIPT', '无法与当前页面通信，请确认扩展已正确加载到此页面');
        return;
      }
      ...
      if (!resumeData) {
        ...
        this.handleAutofillError('NO_RESUME_DATA', '无法获取简历数据，请先完善个人信息');
        return;
      }
      ...
      if (error.message && error.message.includes('Could not establish connection')) {
        ...
        this.handleAutofillError('SEND_ERROR', `发送消息失败: ${error.message}`);
      }
    }
  }
  ...
  async getProfileData() {
    try {
      ...
      return {
        ...
        salary: '15000-20000',
        selfIntroduction: '我是一名热爱编程的软件工程师，具有丰富的前端和后端开发经验。',
        introduction: '我是一名热爱编程的软件工程师，具有丰富的前端和后端开发经验。',
        description: '我是一名热爱编程的软件工程师，具有丰富的前端和后端开发经验。',
        resumeText: '我是一名热爱编程的软件工程师，具有丰富的前端和后端开发经验。',
  ...
  async loadResumeData() {
    try {
      ...
      const currentCompanyInput = document.getElementById('edit-current-company');
      const expectedPositionInput = document.getElementById('edit-expected-position');
      ...
      const projectExperienceTextarea = document.getElementById('edit-project-experience');
      const selfIntroductionTextarea = document.getElementById('edit-self-introduction');
      ...
      if (graduationYearInput) graduationYearInput.value = profileData.graduationYear || '';
      if (currentCompanyInput) currentCompanyInput.value = profileData.currentCompany || '';
      if (expectedPositionInput) expectedPositionInput.value = profileData.expectedPosition || '';
      if (expectedCityInput) expectedCityInput.value = profileData.expectedCity || '';
      ...
      if (projectExperienceTextarea) projectExperienceTextarea.value = profileData.projectExperience || '';
      if (selfIntroductionTextarea) selfIntroductionTextarea.value = profileData.selfIntroduction || '';
