...
class AutoFillBackground {
  ...
  setupMessageListener() {
    chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
      console.log('[JianCareer AutoFill] Background received message:', {
        type: message.type,
        from: sender.tab ? `tab ${sender.tab.id} (${sender.tab.url})` : 'extension',
        timestamp: new Date().toISOString(),
        payload: message.payload ? 'has payload' : 'no payload'
      });
      ...
      try {
        ...
        default:
          console.log('[JianCareer AutoFill] Unknown message type:', message.type);
          sendResponse({ success: false, error: 'Unknown message type' });
          break;
      }
      } catch (error) {
        console.error('[JianCareer AutoFill] Message handling error:', {
          type: message.type,
          error: error.message,
          stack: error.stack
        });
  ...
  setupInstallListener() {
    chrome.runtime.onInstalled.addListener((details) => {
      if (details.reason === 'install') {
        ...
        // 打开教学引导页面
        chrome.tabs.create({ url: chrome.runtime.getURL('mock-form.html') });
      ...
      if (Object.keys(profiles).length === 0) {
        const defaultProfile = {
          ...
          data: {
            ...
            education: '本科',
            salary: '15000',
            resume: '具有丰富的前端开发经验，熟练掌握HTML、CSS、JavaScript等技术栈。'
          },
        ...
        console.log('[JianCareer AutoFill] Default profile created');
      }
    } catch (error) {
      console.error('[JianCareer AutoFill] Failed to initialize default profile:', error);
    }
  }
